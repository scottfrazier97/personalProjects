...
# sample size per sample for 1st distribution
n1 = 50

# sample size per sample for 2nd distribution (we will compare these distribuions)
n2 = 100 

#number of samples and thus number of xbars we will generate.
simulations = 10000

# mean parameter for use with normal distribuions
mu = 2

# standard deviation parameter for use with normal distribuions
sigma = 2

fls_hist = rchisq(simulations, df = 2)

library(RColorBrewer)
coul = brewer.pal(5, "Blues") 
hist(fls_hist, col = coul, xlim = c(0,20), xlab = "", ylab = "", main = "FLS_HIST")

round(mean(fls_hist),digits = 2)
sd(fls_hist)
## Data Holder
```{r}
...
## Simulate and Store
Generate 10000 samples each of size 10 and find the mean of each sample.  Then store each mean in the xbar_holder vector.
